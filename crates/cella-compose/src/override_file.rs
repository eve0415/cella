...
pub fn generate_override_yaml(config: &OverrideConfig) -> String {
    ...
    // host.docker.internal does not resolve inside containers on colima
    // (Lima VM) or bare Linux Docker without an explicit /etc/hosts entry.
    // Harmless on Docker Desktop / OrbStack.
