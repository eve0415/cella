...
///
/// `pub` (within the private `exec` module) so `up` can reuse the same label
/// computation when performing a best-effort container re-find on failure (for
/// the JSON error envelope's `containerId` field).
pub fn spec_identity_labels(workspace_root: &Path, config_path: &Path) -> [String; 2] {
    [
        format!(
