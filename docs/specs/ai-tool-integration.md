...
Installing bubblewrap is not by itself enough for a working Codex sandbox. Under Docker's default `MaskedPaths` and `ReadonlyPaths` the sandbox cannot mount a fresh procfs, so a fully functional sandbox additionally requires `"securityOpt": ["systempaths=unconfined"]` in devcontainer.json. That option removes Docker's default masked and read-only paths, which exposes `/proc/sysrq-trigger`, `/proc/kcore`, `/proc/keys`, `/proc/interrupts` and `/sys/firmware` inside the container. After the Codex install step, cella runs `codex sandbox -- /bin/true` as the remote user and warns with that remedy when the probe fails. The probe is given a throwaway `CODEX_HOME` that is removed afterwards, so the helper binaries Codex materializes on startup never reach the host `~/.codex` that `tools.codex.forward_config` bind-mounts into the container.
...
- **A hub with no valid source stays silent, and a container cannot seed it.** If the host document is absent or malformed when the daemon starts, its canonical is `{}` — indistinguishable from a genuinely empty document — so it pushes nothing rather than overwrite a container's good copy. It seeds as soon as the host file becomes readable or a container sends a non-empty patch. A container that *holds* content but never edits it does not seed the hub: its re-announce is an empty patch, because its file matches its own baseline. The host file therefore stays missing until something writes it.
