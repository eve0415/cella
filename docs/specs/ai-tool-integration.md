...
**Known open defects (not accepted trade-offs).** Found by review, characterized but deliberately not fixed in the change that introduced this design — each needs restructuring a mechanism outside it:

- **The create-time seed rewrites paths by raw `sed`, not by the codec.** It substitutes globally, so a workspace of `/src/app` also rewrites a foreign entry for `/src/application`, and it breaks on paths containing `&` or `|`. `PathMap` then rejects the mangled value at the component boundary, so the normalized diff can push a spurious deletion/addition to the host manifest. The fix is to compute the seed host-side with `to_local(to_canonical(…))` and upload it, leaving the container step to do only the symlinking it uniquely needs a container for.
- **The compose path advertises a workspace pair it cannot verify.** Compose ignores `workspaceMount` — the service's own volumes define the mapping — so a service backing `workspaceFolder` with a named volume, or binding a source other than `workspace_root`, gets a container↔host pair that does not exist. The agent then rewrites `projectPath` to an unrelated host directory *and uses it as the normalized install-context key*, so a wrong path does not merely mislabel an entry: it creates a distinct merge slot. The single-container path does not have this problem — it derives the pair from the effective mount and omits it for a volume-backed workspace.
- **Plugin hubs are built once, at daemon startup.** If `~/.claude/plugins` does not exist then, no hub is ever created for either manifest, and every subsequent manifest patch is dropped silently — with no host watcher — until the daemon restarts.
