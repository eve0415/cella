...
- Each canonical state carries a monotonic revision. The daemon serializes its transactions but fans out after releasing the document lock, so a newer push can overtake an older one; an agent drops any revision below the last it applied, and resets that watermark on every (re)connect because revisions restart with the daemon.
...
- On (re)connect an agent announces, per document and *before* it starts processing pushes, its whole document (`ConfigDocSnapshot`) followed by the patch against its baseline; the daemon merges them and always replies with canonical, which is how a container that changed nothing while disconnected learns what it missed. A document that could not be read at that moment is not announced at all, and the next edit forwarded for it carries the whole document again rather than a delta the daemon may have nothing to apply to. Until that edit, such a container is unrepresented in the hub, so a push that follows a peer's announcement replaces its copy wholesale — the same stance the writer already takes, where an unreadable file has nothing worth preserving across a push.
- Opt-in is enforced on **both** directions: the daemon only broadcasts to agents that advertised sync, and it only *ingests* a container's `ConfigDocPatch` or `ConfigDocSnapshot` if that container opted in. A container without config forwarding can neither read pushes nor write any host document nor reach peers.
...
- **The compose path resolves its workspace mapping from the service's own `volumes:`**, since compose ignores `workspaceMount`. A workspace backed by a named volume gets no mapping and `projectPath` passes through untranslated.
- **A `projectPath` belonging to another container's workspace is left untranslated.** It matches neither mapping, so it passes through in both directions rather than being mangled, and is inert on any side that isn't that container. Because patches only carry what a container actually edited, such an entry is not re-pushed while untouched — but a container that *does* edit one forwards the untranslated path, and entries already on the host from before this change are not retroactively cleaned.
- **A hub with no valid source stays silent, and only a whole document seeds it.** If the host document is absent or malformed when the daemon starts, its canonical is `{}` — indistinguishable from a genuinely empty document — so it neither pushes nor writes the host file rather than overwrite a container's good copy. It seeds as soon as the host file becomes readable, or as soon as a container announces a non-empty document. A patch never seeds it, however much it carries: a patch is a delta against a baseline that lives in the container and outlives the daemon, so canonical after applying one is a fragment of that container's document — and a push is applied wholesale, so pushing the fragment back would delete every key the patch happened not to mention, in the sender and in every peer. An unseeded hub does not even keep the patches it receives: a fragment left in canonical would be published by whatever seeds the hub next, as if it belonged to the announced document. Because the containers' baselines can outlive the daemon's memory of them while the host file cannot seed it, such a hub accepts `ConfigDocSnapshot` from any container for the rest of its life — including after a host file appears mid-life — so a container reconnecting with a stale copy can put an older value back for a key they both hold. That is the accepted direction of the trade: an announcement merges key by key and never removes anything (a `null` inside it is a value, not a tombstone), whereas refusing announcements once anything had seeded the hub would leave every container that has not yet announced facing a canonical that cannot contain its keys, to be overwritten wholesale by the first push. A hub that did read the host file at startup ignores announcements outright, so there a container reconnecting with a stale copy cannot re-assert it over a peer's newer state.
...
**Path translation at create time.** The two manifests are computed *on the host*, through the same codec the live sync uses (`to_canonical` → repair any foreign writer's `.claude` home → `to_local` with this container's `PathMap`), and uploaded. They are the only entries under `plugins/` that are copies rather than symlinks, because their absolute paths differ per container. The workspace half of the mapping is read back from the container's own pinned environment, so the seed and the agent are provably the same mapping. A raw text substitution is deliberately not used: it rewrites any matching substring, so a workspace of `/src/app` would also mangle a foreign entry for `/src/application`, and it breaks on a path containing `&` or `|`.
...
The Codex CLI keeps six runtime SQLite databases (`state_5`, `thread_history_1`, `memories_1`, `queue_1`, `logs_2` and `goals_1`) directly inside `CODEX_HOME`, and opens every one of them in WAL mode.
...
cella therefore sets `CODEX_SQLITE_HOME` to `$HOME/.codex-db`, a container-local path outside the bind mount, so every database the CLI opens is opened by one kernel only.
...
Only the databases are split, and they are a derived cache rather than the system of record: Codex reconstructs its thread index from the forwarded `sessions/` rollouts, so session history survives a container rebuild.
...
Both the create-time seed and the live sync use the **same** field-targeted rewriting on parsed JSON (`installPath`, `installLocation`, `projectPath`), with prefixes matched only at path-component boundaries. `repair_claude_home` additionally normalizes any previous writer's home (`/home/*/.claude`, `/Users/*/.claude`, `/root/.claude`) to this host's, so a manifest written by a container with a different remote user still resolves — on the host side, where it repairs the canonical document rather than only one container's copy.
